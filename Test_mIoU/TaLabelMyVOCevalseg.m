...
for i=1:length(gtids)
    ...
    gtfile = sprintf('/home/zqq/zqqHD/DataSet/Pascal/VOCdevkit/VOC2012/SegmentationClassAug_strong/%s.png',imname);
    %gtfile = sprintf(VOCopts.seg.clsimgpath,imname);
    [gtim,map] = imread(gtfile);    
    gtim = double(gtim);
    
    % results file
    resfile = sprintf(VOCopts.seg.clsrespath,imname);
    try
...
fprintf('-------------------------\n');
fprintf('Average intersection: %6.3f%%\n',gtjresjsum);
fprintf('Average union: %6.3f%%\n',denomsum);
