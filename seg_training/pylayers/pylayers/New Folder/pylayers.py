...
class DSRGLayer(caffe.Layer):
    ...
    def forward(self, bottom, top):
       ...
       # np.save('/home1/zqqHD/Smantic_Segmentation/DSRG-master/seedcbefore.npy',seed_c)
       # np.save('/home1/zqqHD/Smantic_Segmentation/DSRG-master/cues.npy',cues)
       # np.save('/home1/zqqHD/Smantic_Segmentation/DSRG-master/imglabels.npy',img_labels)

      #  f = open("/home1/zqqHD/Smantic_Segmentation/DSRG-master/cue3.txt",'a')
      #  f.writelines('before\n')
      #  print(img_labels.shape)
      #  print(img_labels.shape[0])
      #  for i in range(img_labels.shape[0]):
      #      label_index=np.where(img_labels[i]==1)[2]
      #      print(label_index)
      #      for l in range(21):
      #          if l in label_index:
      #              if seed_c[i][l].max()==0:
      #                  f.writelines('less '+ str(l)+' ')
      #                  print('beforeless'+str(i)+'--'+str(l))
      #          else:
      #              if seed_c[i][l].max()>0:
      #                  f.writelines('more '+str(l)+' ')
      #                  print('beforemore'+str(i))
      #      f.writelines('\n')
      #  #f.close()
      #  print('------------------------------')

      #  for i in range(img_labels.shape[0]):
      #      label_id = np.where(img_labels[i] == 1)[2]
      #      for l in label_id:
      #          if seed_c[i][l].max() == 0:
      #              #seed_c[i][l] = cues[i][l]
      #              kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
      #              seed_c[i][l] = cv2.dilate(cues[i][l], kernel)
       # np.save('/home1/zqqHD/Smantic_Segmentation/DSRG-master/seedcafer.npy',seed_c)
       # f.writelines('after\n')
       # for i in range(img_labels.shape[0]):
       #     label_index=np.where(img_labels[i]==1)[2]
       #     for l in range(21):
       ...
       #             if seed_c[i][l].max()==0:
       #                 f.writelines('less '+ str(l)+' ')
       #                 print('afterless'+str(i)+'--'+str(l))
       #         else:
       #             if seed_c[i][l].max()>0:
       #                 f.writelines('more '+str(l)+' ')
       #                 print('aftermore'+str(i))
       #     f.writelines('\n')
       # f.close()
       # print('---------------------------------')
...
class AnnotationLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        self.data_file = cPickle.load(open(osp.join(this_dir, './', 'localization_cues', self._cue_name)))
    ...
    def forward(self, bottom, top):
        ...
        for i, image_id in enumerate(bottom[0].data[...]):
            ...
            cues_i_rzero=cues_i[0].astype('int64')
            cues_i_rone=cues_i[1].astype('int64')
            cues_i_rtwo=cues_i[2].astype('int64')
            top[1].data[i, cues_i_rzero, cues_i_rone, cues_i_rtwo] = 1.0
