...
def load_image_label_list_from_npy(img_name_list,label_npy):

    cls_labels_dict = np.load(label_npy).item()
...
class VOC12ClsDataset(VOC12ImageDataset):
    ...
    def __init__(self, img_name_list_path,label_npy, voc12_root, transform=None):
        super().__init__(img_name_list_path, voc12_root, transform)
        self.label_list = load_image_label_list_from_npy(self.img_name_list,label_npy)
...
class VOC12ClsDatasetMSF(VOC12ClsDataset):
    ...
    def __init__(self, img_name_list_path,label_npy, voc12_root, scales, inter_transform=None, unit=1):
        super().__init__(img_name_list_path, label_npy,voc12_root, transform=None)
        self.scales = scales
        self.unit = unit
