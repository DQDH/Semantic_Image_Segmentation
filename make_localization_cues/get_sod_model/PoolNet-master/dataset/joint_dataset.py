...
class ImageDataTest(data.Dataset):
    def __init__(self, data_root, image_name):
    ...
    def __getitem__(self, item):
        image, im_size = load_image_test(os.path.join(self.data_root, self.image_list[item]+'.jpg'))
...
def get_loader(config, mode='train', pin=False):
    ...
    if mode == 'train':
        ...
        dataset = ImageDataTest(config.test_root, config.image_name)
        data_loader = data.DataLoader(dataset=dataset, batch_size=config.batch_size, shuffle=shuffle, num_workers=config.num_thread, pin_memory=pin)
    return data_loader
